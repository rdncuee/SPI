...
define([
	...
	'tpl!ScatterPlotInteraction/creator/tpl/propertiesForm',
	'lodash',
	'jquery',
	'jqueryui',
	'ScatterPlotInteraction/runtime/js/i18n.translation'
	...
	var ScatterPlotStateQuestion = stateFactory.extend(Question, function () {
		...
		var Scontainer = this.widget.$container,
			Sprompt = Scontainer.find('.prompt'),
			SrawDataCaption = Scontainer.find('.rawDataCaption'),
			SscatterPlotCaption = Scontainer.find('.scatterPlotCaption');
	 ...
	 * 初期化
	 */
	ScatterPlotStateQuestion.prototype.initForm = function () {
		var _widget = this.widget,
			...
			scatterPlotCaption = (interaction.prop('scatterPlotCaption') || ((i18nLocale === LANG_JP) ? '散布図' : 'Scatter Plot')),
			plotExcludable = (interaction.prop('plotExcludable') || false),
			plotShowOrHide = (interaction.prop('plotShowOrHide') || 'hide'),
			plotShow = (plotShowOrHide === 'show'),
			plotHide = (plotShowOrHide === 'hide'),
			xAxis = (interaction.prop('xAxis') || ''),
			xAxisList = {},
			yAxis = (interaction.prop('yAxis') || ''),
			yAxisList = {},
			groupBy = (interaction.prop('groupBy') || ''),
			groupByList = {},
			colorScheme = (interaction.prop('colorScheme') || ['#a6cee3', '#1f78b4', '#b2df8a', '#33a02c', '#fb9a99', '#e31a1c', '#fdbf6f', '#ff7f00']),
			colorSchemeList = {},
			graphType = (interaction.prop('graphType') || 'scatter'),
			graphTypeScatterPlot = (interaction.prop('graphTypeScatterPlot') || (graphType === 'scatter')),
			graphTypeBubbleChart = (interaction.prop('graphTypeBubbleChart') || (graphType === 'bubble')),
			bubbleChartValue = (interaction.prop('bubbleChartValue') || ''),
			bubbleChartValueList = {},
			showCorrelation = (interaction.prop('showCorrelation') || false),
			correlationEditable = (interaction.prop('correlationEditable') || false),
			showRegressionLine = (interaction.prop('showRegressionLine') || false),
			regressionLineEditable = (interaction.prop('regressionLineEditable') || false),
			showRsquared = (interaction.prop('showRsquared') || false),
			RsquaredEditable = (interaction.prop('RsquaredEditable') || false);

		/**
		 * カラム名リストのチェック(JSONで格納されている場合はparseしておく)
		 */
		...
		_.each(csvColumnNames, function (csvColumnName) {
			// x軸
			xAxisList[csvColumnName] = {
				label: csvColumnName,
				selected: (csvColumnName === xAxis)
			};
			// ｙ軸
			yAxisList[csvColumnName] = {
				label: csvColumnName,
				selected: (csvColumnName === yAxis)
			};
			// グループ設定
			groupByList[csvColumnName] = {
				label: csvColumnName,
				selected: (csvColumnName === groupBy)
			};
			// バブルの値設定
			bubbleChartValueList[csvColumnName] = {
				label: csvColumnName,
				selected: (csvColumnName === bubbleChartValue)
			};
		});
		...
		$.each(brewerColorSchemes, function (key, value) {
		...
		Sform.html(formTpl({
			...
			scatterPlotCaption: scatterPlotCaption,
			plotExcludable: plotExcludable,
			plotShowOrHide: plotShowOrHide,
			plotShow: plotShow,
			plotHide: plotHide,
			xAxis: xAxis,
			xAxisList: xAxisList,
			...
			graphType: graphType,
			graphTypeScatterPlot: graphTypeScatterPlot,
			graphTypeBubbleChart: graphTypeBubbleChart,
			bubbleChartValue: bubbleChartValue,
			bubbleChartValueList: bubbleChartValueList,
			showCorrelation: showCorrelation,
			correlationEditable: correlationEditable,
			showRegressionLine: showRegressionLine,
			regressionLineEditable: regressionLineEditable,
			showRsquared: showRsquared,
			RsquaredEditable: RsquaredEditable
		}));
		...
		formElement.setChangeCallbacks(Sform, interaction, {
			...
			scatterPlotCaption: function scatterPlotCaption(interaction, value) {
				console.log("scatterPlotCaption 変更しました " + value);
				// update the pci property value:
				interaction.prop('scatterPlotCaption', value);
				// update rendering
				_widget.refresh();
			},
			plotExcludable: function plotExcludable(interaction, value) {
				console.log("plotExcludable 変更しました " + value);
				// update the pci property value:
				interaction.prop('plotExcludable', value);
				// update rendering
				_widget.refresh();
			},
			plotShowOrHide: function plotShowOrHide(interaction, value) {
				console.log("plotShowOrHide 変更しました " + value);
				// update the pci property value:
				interaction.prop('plotShowOrHide', value);
				interaction.prop('plotShow', (value === 'show'));
				interaction.prop('plotHide', (value !== 'show'));
				// update rendering
				_widget.refresh();
			},
			xAxis: function xAxis(interaction, value) {
				console.log("xAxis 変更しました " + value);
				// update the pci property value:
				interaction.prop('xAxis', value);
				// update rendering
				_widget.refresh();
			},
			yAxis: function yAxis(interaction, value) {
				console.log("yAxis 変更しました " + value);
				// update the pci property value:
				interaction.prop('yAxis', value);
				// update rendering
				_widget.refresh();
			},
			groupBy: function groupBy(interaction, value) {
				console.log("groupBy 変更しました " + value);
				// update the pci property value:
				interaction.prop('groupBy', value);
				// update rendering
				_widget.refresh();
			},
			...
			bubbleChartValue: function bubbleChartValue(interaction, value) {
				console.log("bubbleChartValue 変更しました " + value);
				// update the pci property value:
				interaction.prop('bubbleChartValue', value);
				// update rendering
				_widget.refresh();
			},
			graphType: function graphType(interaction, value) {
				...
				interaction.prop('graphTypeScatterPlot', (value === 'scatter') ? true : false);
				interaction.prop('graphTypeBubbleChart', (value === 'scatter') ? false : true);
				// update rendering
				_widget.refresh();
			},
			showCorrelation: function showCorrelation(interaction, value) {
				console.log("showCorrelation 変更しました " + value);
				// update the pci property value:
				interaction.prop('showCorrelation', value);
				// update rendering
				_widget.refresh();
			},
			correlationEditable: function correlationEditable(interaction, value) {
				console.log("correlationEditable 変更しました " + value);
				// update the pci property value:
				interaction.prop('correlationEditable', value);
				// update rendering
				_widget.refresh();
			},
			showRegressionLine: function showRegressionLine(interaction, value) {
				console.log("showRegressionLine 変更しました " + value);
				// update the pci property value:
				interaction.prop('showRegressionLine', value);
				// 回帰直線の表示・編集共にfalseの場合は、決定係数の設定もfalseにする
				if (!interaction.prop('showRegressionLine') && !interaction.prop('regressionLineEditable')) {
					interaction.prop('showRsquared', false);
					interaction.prop('RsquaredEditable', false);
				}
				// update rendering
				_widget.refresh();
			},
			regressionLineEditable: function regressionLineEditable(interaction, value) {
				console.log("regressionLineEditable 変更しました " + value);
				// update the pci property value:
				interaction.prop('regressionLineEditable', value);
				// 回帰直線の表示・編集共にfalseの場合は、決定係数の設定もfalseにする
				if (!interaction.prop('showRegressionLine') && !interaction.prop('regressionLineEditable')) {
					interaction.prop('showRsquared', false);
					interaction.prop('RsquaredEditable', false);
			...
			showRsquared: function showRsquared(interaction, value) {
				console.log("showRsquared 変更しました " + value);
				// update the pci property value:
				interaction.prop('showRsquared', value);
				// update rendering
				_widget.refresh();
			},
			RsquaredEditable: function RsquaredEditable(interaction, value) {
				console.log("RsquaredEditable 変更しました " + value);
				// update the pci property value:
				interaction.prop('RsquaredEditable', value);
				// update rendering
				_widget.refresh();
			},
		...
		// scatterPlotCaptionの初期設定
		if (interaction.prop('scatterPlotCaption') === "") {
			Sform.find("input.scatterPlotCaption").val(scatterPlotCaption).change();
		}
	};
	return ScatterPlotStateQuestion;
});
